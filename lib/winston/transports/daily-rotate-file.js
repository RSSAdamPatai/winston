...
DailyRotateFile.prototype.open = function (callback) {
  ...
  if (this.opening) {
    ...
    return callback(true);
      ...
      (this._year < now.getFullYear() || this._month < now.getMonth() || this._date < now.getDate() || this._hour < now.getHours() || this._minute+1 < now.getMinutes())) {
    //
...
DailyRotateFile.prototype._createStream = function () {
  ...
  (function checkFile (target) {
    ...
    fs.stat(fullname, function (err, stats) {
      ...
      var now = new Date();
      if (self._year < now.getFullYear() || self._month < now.getMonth() || self._date < now.getDate() || self._hour < now.getHours() || self._minute+1 < now.getMinutes()) {
